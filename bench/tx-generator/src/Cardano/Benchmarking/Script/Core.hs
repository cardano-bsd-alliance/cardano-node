...
runBenchmark :: AnyCardanoEra -> WalletName -> SubmitMode -> ThreadName -> RunBenchmarkAux -> TPSRate -> ActionM ()
runBenchmark era sourceWallet submitMode threadName extraArgs tps
  = withEra era $ runBenchmarkInEra sourceWallet submitMode threadName extraArgs tps
...
runBenchmarkInEra sourceWallet submitMode (ThreadName threadName) shape tps era = do
  ...
  let
    ...
    txGenerator = genTx protocolParameters (TxInsCollateralNone, []) (mkFee (auxFee shape)) metadata
...
createChangeInEra sourceWallet dstWallet submitMode payMode value count _era = do
  ...
  (toUTxO, addressMsg) <- case payMode of
    ...
    PayToScript scriptSpec -> do
      ...
      return ( PlutusExample.mkUTxOScriptList networkId (script, scriptData) witness Confirmed
